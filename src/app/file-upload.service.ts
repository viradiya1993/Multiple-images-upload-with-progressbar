  ...
  upload() {
    ...
    while ( ind < this.files.length ) {
      ...
      reader.onload = (_event) => {
        ...
        console.log('-----------------------------------------', imgUrl);
        this.subject.next({name: fileName, type: -1, progress: 0, imgUrl: imgUrl, tag: filetag});
      };
      ind++;
    }
  ...
  uploadFile(index: number) {
    ...
    this.http.post(this.apiUrl, uploadData, {
      ...
      .subscribe(event => {
        ...
        if (event.type === 0) {
          this.subject.next({name: this.files[index].file.name, type: event.type, progress: 0});
          this.uploadingFileCount++;
          this.currentIndex++;
          // console.log(this.files[index].name);
          // console.log('-----------------', this.uploadingFileCount);
          if (this.uploadingFileCount < 3 ) {
            this.sbj.next();
          }
        } else if (event.type === 1 ) {
          // @ts-ignore
          this.subject.next({name: this.files[index].file.name, type: event.type, progress: Math.round(100 * event.loaded / event.total)});
        } else  if (event.type === 4) {
          this.subject.next({name: this.files[index].file.name, type: event.type, progress: 100});
          this.uploadingFileCount--;
